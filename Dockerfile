...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libicu-dev libsodium-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc && rm -rf /var/lib/apt/lists/*
...
EXPOSE 80
CMD R -e "options('shiny.port'=80,shiny.host='0.0.0.0');SkeletonShinyDashboardLogin::run_app()"
